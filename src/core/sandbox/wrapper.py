...
def _get_protected_paths(cwd: str) -> list[str]:
    ...
    Corresponds to sandbox-adapter.ts:230-255:
    - .my-claw.toml (project config)
    - ~/.config/my-claw/config.toml (global config)
    - CLAUDE.md (should not be modified by sandbox)
    """
    paths = []
    local_config = Path(cwd) / ".my-claw.toml"
    if local_config.exists():
        paths.append(str(local_config))
    global_config = Path.home() / ".config" / "my-claw" / "config.toml"
