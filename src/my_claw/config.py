...
_ENV_MODEL = "MY_CLAW_MODEL"
_ENV_MAX_TOKENS = "MY_CLAW_MAX_TOKENS"
_ENV_MEMORY_DIR = "MY_CLAW_MEMORY_DIR"
_ENV_PROVIDER = "MY_CLAW_PROVIDER"
_ENV_EFFORT = "MY_CLAW_EFFORT"
_ENV_BUDDY_MODEL = "MY_CLAW_BUDDY_MODEL"
_DEFAULT_CONFIG_PATHS = (
    Path.home() / ".config" / "my-claw" / "config.toml",
    Path.cwd() / ".my-claw.toml",
)
...
class AppConfig:
    ...
    memory_dir: Path = Path.home() / ".my-claw" / "memory"
...
def load_app_config(args: Namespace) -> AppConfig:
    ...
    memory_dir = Path(raw_memory_dir).expanduser() if raw_memory_dir else Path.home() / ".my-claw" / "memory"
...
def _load_env_values() -> dict[str, Any]:
    ...
    values["anthropic_api_key"] = os.environ.get("ANTHROPIC_API_KEY", _DEFAULT_ANTHROPIC_API_KEY)
    values["anthropic_base_url"] = os.environ.get("ANTHROPIC_BASE_URL", _DEFAULT_ANTHROPIC_BASE_URL)
