...
def companion_user_id() -> str:
    ...
    Since my-claw has no OAuth, use username@hostname as the seed.
    Same user on same machine always gets the same companion.

    Set MY_CLAW_BUDDY_SEED env var to override (useful for testing).
    """
    import os
    override = os.environ.get('MY_CLAW_BUDDY_SEED')
    if override:
        return override
    try:
